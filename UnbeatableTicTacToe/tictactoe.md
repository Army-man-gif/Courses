...
These lecture notes have the material on trees from [Data1](Data1.md) and [Data2](Data2.md) as a prerequisite.
...
by [Graham Hutton](http://www.cs.nott.ac.uk/~pszgmh/), Cambridge University Press, 2016, with annotations and additions by the module lecturer. **You are required to read chapter 11 [the book](https://rl.talis.com/3/bham/lists/B8EC8CB3-9A35-6677-9416-A2FB63C60D88.html)
 for a full account.** This handout is just a summary.
...
This material is not in the adopted textbook and is due to your lecturers.
...
We will provide an improved version which is **6 times faster** - almost an
...
Because `minimum` and `maximum` are defined from `min` and `max` using `fold`, they inherit the laziness of `min` and `max` and so become faster. Sometimes laziness pays off.
...
```
> leavescount (alphabeta (gametree empty O))
38856
> leavescount (gametree empty O) `div` leavescount (alphabeta (gametree empty O))
6
```
This means that the search space is 6 times smaller when we apply (directly or indirectly) alpha-beta prunning: the full game tree has 255168, but the alpha-beta pruned tree has only 38856 leaves. This explains why `bestmoves empty O` is much faster that `bestmove empty O`.

You can replace `minimax` by `alphabeta` in the definition of
`bestmove` to get `bestmove empty O` computed in a fraction of a
second.
