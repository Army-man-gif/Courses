...
-- Check if a binary tree is a BST (quadratic time)
...
-- Check BST property via in-order traversal (linear time)
isBST' :: Ord a => BT a -> Bool
...
-- Create full/balanced BST from range
...
-- Check winning/losing positions
...
-- Permutation tree (using equality)
...
-- Faster version using difference lists
...
-- Permutation tree without equality constraint
permTree2 :: [a] -> Tree a
